...
const ALLOWED_TELEGRAM_USERS = new Map([
  ["5328212518", "Папа"],
  ["775978948", "Мама"],
]);

function isAllowedUser(ctx: Context): boolean {
  return ALLOWED_TELEGRAM_USERS.has(String(ctx.from?.id ?? ""));
}

async function replyAccessDenied(ctx: Context): Promise<void> {
  await ctx.reply(
    "Доступ закрыт. Amir Bot работает только для мамы и папы Амира.",
  );
}
...
export function registerCommands(
  ...
  bot.command("today", async (ctx) => {
    ...
    const message =
      await dependencies.todaySummaryService.getSummaryMessage(telegramUserId);
